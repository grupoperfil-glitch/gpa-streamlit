...
# Padrões de mapeamento (pode ajustar na UI do app)
ESQUEMA_PADRAO = {
    "student": "Nome",
    "turma": "Turma",
...
def tabela_gpa_padrao() -> pd.DataFrame:
    """Tabela padrão de Média→GPA (faixas inclusivas [min, max])."""
    dados = [
    ...
    return (
        pd.DataFrame(dados, columns=["min", "max", "gpa"])
        .sort_values(["min", "max"])
